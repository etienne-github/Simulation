...
public class Herbivorious extends Animal {
	...
	public void  initializePerception(){
		...
		int perceptionPoint = (int) Math.round(toCase(Math.max(smellPoint, visionPoint)));
	...
	private Integer[] getNearbyFood() {
		...
		double minDistance=toCase(movePoint);
		
		double currentDistance;
		for(PerceptionTableCell[] i:perception){
			for(PerceptionTableCell j:i){
				// un herbivore considère l'herbe mangeable quand il peut manger au moins
				// la moitié d'une portion normale (portion normale = toStep(weightConsomeByDay)*2)
				if(j.getAmount() >= toStep(weightConsumeByDay)){
	...
	private Double moveRandom(SparseGrid2D yard) {
		...
		float dX = (float) (Math.cos(randomAngle)*toCase(movePoint));
		float dY = (float) (Math.sin(randomAngle)*toCase(movePoint));
	...
	private void eat(int VegetationX, int VegetationY, Float[][] floats, SimulationModel SimModel) {
		...
		// * 2 en supposant qu'un herbivore mange la moitié de son temps
		float consumes=SimModel.consumeVegetationAt(VegetationX, VegetationY, toStep(weightConsumeByDay)*2);
	...
	public boolean isOnFood(){
		// un herbivore considère l'herbe mangeable quand il peut manger au moins
		// la moitié d'une portion normale (portion normale = toStep(weightConsomeByDay)*2)
		if(simModel.getVegetationAt((int)getX(), (int)getY())> toStep(weightConsumeByDay)){
			return true;
		}else{
			return false;
		}
	...
	public void printPerception(){
		int perceptionPointCase = (int)Math.round(toCase(Math.max(smellPoint, visionPoint)));

		for(int i=0;i<perceptionPointCase*2;i++){
			for(int j=0;j<perceptionPointCase*2;j++){
				if((i==perceptionPointCase)&&(j==perceptionPointCase)){
					System.out.print("X ");
				}else{
